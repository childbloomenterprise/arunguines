...
import { ArrowUpRight, Copy, Plus } from "../icons";
...
export function BookingForm({ initialShow = "", initialEvent = "", initialLocation = "", compact = false }: BookingFormProps) {
  ...
  const [status, setStatus] = useState("Nothing sent until WhatsApp.");
  ...
  return (
    <form className={`booking-form ${compact ? "is-compact" : ""}`} onSubmit={submit}>
      <div className="form-head"><span>Enquiry</span><strong>Date. Place. Stage.</strong></div>
      ...
      <details className="booking-optional"><summary><Plus />Audience + requirements</summary><label htmlFor={`audience-${compact}`}>Expected audience<input id={`audience-${compact}`} name="audience" inputMode="numeric" placeholder="Approximate size" /></label><label htmlFor={`notes-${compact}`}>Requirements<textarea id={`notes-${compact}`} name="notes" rows={compact ? 2 : 4} placeholder="Time, languages, venue..." /></label></details>
      <button className="button button-brass" type="submit">WhatsApp <ArrowUpRight /></button>
