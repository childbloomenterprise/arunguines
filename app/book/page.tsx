...
type BookPageProps = { searchParams: Promise<{ show?: string; event?: string; location?: string; country?: string; source?: string }> };
...
export default async function BookPage({ searchParams }: BookPageProps) {
  ...
  return <PageFrame>
    ...
    <section className="booking-layout section-shell first-task">
      <BookingForm initialShow={params.show ?? ""} initialEvent={params.event ?? ""} initialLocation={params.location ?? ""} initialCountry={params.country ?? ""} source={params.source ?? "/book"} />
